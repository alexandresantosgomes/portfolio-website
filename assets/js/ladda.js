...
(this, function( Spinner ) {
	...
	function create( button , options) {
		...
		if( !button.hasAttribute( 'data-style' ) ) {
			if(options != 'undefined' && options != null && options.style != null){
				button.setAttribute( 'data-style', options.style);
			}else
	...
	*/
	...
	function bind( target, options ) {
		...
		for( var i = 0; i < targets.length; i++ ) {
			if(options != 'undefined' && options.style != null){
				targets[i].setAttribute( 'data-style', options.style);
			}
